...
transcript_rnaseq_limmavoom_v <- limma::voom(dge_transcript_rnaseq_filtered,  model_matrix$transcript_rnaseq, plot=TRUE)
```
...
# remove heteroscedascity from count data and plot mean variance
gene_rnaseq_limmavoom_v <- limma::voom(dge_gene_rnaseq_filtered,  model_matrix$gene_rnaseq, plot=TRUE)
```
...
transcript_rnaseq_limmavoom_vfit <- limma::lmFit(transcript_rnaseq_limmavoom_v, model_matrix$transcript_rnaseq)

# estimate contrast for each transcript
transcript_rnaseq_limmavoom_vfit <- limma::contrasts.fit(transcript_rnaseq_limmavoom_vfit, contr_matrix$transcript_rnaseq)
...
```{r, eval = config$gene_rnaseq, out.width = "100%"}
# fit model to the expression values for each gene/transcript
gene_rnaseq_limmavoom_vfit <- limma::lmFit(gene_rnaseq_limmavoom_v, model_matrix$gene_rnaseq)

# estimate contrast for each gene
gene_rnaseq_limmavoom_vfit <- limma::contrasts.fit(gene_rnaseq_limmavoom_vfit, contr_matrix$gene_rnaseq)
...
limma_voom_results_1 <- limma_voom_results_1 %>%
  ...
  dplyr::mutate(dataset = gsub("_.*", "", source)) %>%
...
```{r, eval = config$transcript_rnaseq, out.width = "100%"}
# plot
data <- deseq_results_1$transcript_rnaseq
W <- data$stat
maxCooks <- base::apply(SummarizedExperiment::assays(deseq$transcript_rnaseq)[["cooks"]], 1, base::max)
idx <- !is.na(W)
base::plot(rank(W[idx]), maxCooks[idx], xlab = "rank of Wald statistic", 
           ylab = "maximum Cook's distance per gene/transcript",
           ylim = c(0, 5), cex = .4, col = rgb(0, 0, 0, .3))
m <- base::ncol(deseq$transcript_rnaseq)
p <- 3
...
```{r, eval = config$gene_rnaseq, out.width = "100%"}
# plot
data <- deseq_results_1$gene_rnaseq
W <- data$stat
maxCooks <- base::apply(SummarizedExperiment::assays(deseq$gene_rnaseq)[["cooks"]], 1, base::max)
idx <- !is.na(W)
base::plot(rank(W[idx]), maxCooks[idx], xlab = "rank of Wald statistic", 
           ylab = "maximum Cook's distance per gene/transcript",
           ylim = c(0, 5), cex = .4, col = rgb(0, 0, 0, .3))
m <- base::ncol(deseq$gene_rnaseq)
p <- 3
graphics::abline(h = qf(.99, p, m - p))
```

```{r, eval = contrasts[1]}
# define which pipeline, differential expression method, dataset and comparison/contrast that was used (info stored in rownames) (loop over all datasets)
